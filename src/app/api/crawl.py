...
class CrawlingPageResponse(BaseModel):
    loc: HttpUrl
    lastmod: Union[str, None] = Field(
        default=None, title="last modified date"
    )
...
@router.post(
        ...
        response_model=List[CrawlingPageResponse],
        )
async def get_pages(crawl_in: CrawlingPageRequest,):
    ...
    return [CrawlingPageResponse(**u) for u in urls]
